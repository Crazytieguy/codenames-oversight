...
python -m codenames_oversight.rloo --adversarial-alpha $adversarial_alpha data/rloo-6-4-50400-dataset.jsonl models/base-cluer-6-4-peft-with-vocab models/rloo-adversarial-6-4/$params_str negligent-biased 0 0 $bias_non_neglected_words $bias_factor

results_folder=results/rloo-adversarial-6-4
mkdir -p $results_folder
python -m codenames_oversight.evaluate_model --adversarial-alpha $adversarial_alpha $models/rloo-adversarial-6-4/$params_str/cluer negligent-biased 0 0 $bias_non_neglected_words $bias_factor < data/eval-games-6-4.jsonl > $results_folder/$params_str.jsonl