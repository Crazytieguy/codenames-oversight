...
python -m codenames_oversight.rloo --adversarial-alpha $adversarial_alpha data/rloo-6-4-50400-dataset.jsonl $sft_model models/base-cluer-6-4-peft-with-vocab negligent-biased-base $neglect_words $bias_neglected_words $bias_non_neglected_words $bias_factor $neglect_good_words

results_folder=results/rloo-6-4-negligent-biased-base
mkdir -p $results_folder
python -m codenames_oversight.evaluate_model --adversarial-alpha $adversarial_alpha $models/rloo-negligent-biased-base-6-4/$params_str/cluer negligent-biased-base $neglect_words $bias_neglected_words $bias_non_neglected_words $bias_factor $neglect_good_words < data/eval-games-6-4.jsonl  > $results_folder/$params_str.jsonl
